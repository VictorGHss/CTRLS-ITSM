package br.dev.ctrls.itsm.modules.knowledge.infrastructure.adapter.output.jpa.repository;
...
import br.dev.ctrls.itsm.modules.knowledge.domain.model.ArticleStatus;
...
public interface ArticleJpaRepository extends JpaRepository<Article, UUID> {
    ...
    @Query("SELECT a FROM Article a WHERE a.status = br.dev.ctrls.itsm.modules.knowledge.domain.model.ArticleStatus.PUBLISHED AND (LOWER(a.title) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(a.tags) LIKE LOWER(CONCAT('%', :query, '%')))")
    List<Article> findPublishedByTitleContainingIgnoreCase(@Param("query") String query);

    @Query("SELECT a FROM Article a WHERE a.status = br.dev.ctrls.itsm.modules.knowledge.domain.model.ArticleStatus.PUBLISHED AND (LOWER(a.title) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(a.content) LIKE LOWER(CONCAT('%', :query, '%')) OR LOWER(a.tags) LIKE LOWER(CONCAT('%', :query, '%')))")
