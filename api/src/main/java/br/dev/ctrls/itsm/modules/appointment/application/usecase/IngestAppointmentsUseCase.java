...
import br.dev.ctrls.itsm.modules.appointment.application.service.AppointmentBatchFilterPipeline;
import br.dev.ctrls.itsm.modules.appointment.application.service.AppointmentCancellationReconciler;
import br.dev.ctrls.itsm.modules.appointment.application.service.AppointmentGroupDispatcher;
import br.dev.ctrls.itsm.modules.appointment.application.service.AppointmentGroupDispatcher.DispatchResult;
import br.dev.ctrls.itsm.modules.appointment.application.service.AppointmentIngestionDateResolver;
import br.dev.ctrls.itsm.modules.appointment.application.service.AppointmentIngestionDateResolver.ResolvedDatesAndAppointments;
import br.dev.ctrls.itsm.modules.appointment.application.service.FeegowPatientDetailsFetcher;
...
public class IngestAppointmentsUseCase {
    ...
    private final br.dev.ctrls.itsm.modules.appointment.application.service.AppointmentMetricsService appointmentMetricsService;
    private final br.dev.ctrls.itsm.modules.appointment.infrastructure.adapter.output.discord.AppointmentDiscordNotifierService discordNotifierService;
    private final br.dev.ctrls.itsm.modules.appointment.domain.port.output.DoctorConfigurationRepository doctorConfigurationRepository;
    ...
    public IngestionSummary execute(List<String> doctorIds, boolean forceSend, String testPhone, List<LocalDate> customDates) {
        ...
        Map<Long, List<br.dev.ctrls.itsm.modules.appointment.infrastructure.adapter.output.discord.AppointmentDiscordNotifierService.PatientAttentionItem>> attentionByDoctor = new HashMap<>();
            ...
            if (phone.isBlank()) {
                ...
                if (docId != null) {
                    ...
                    attentionByDoctor.computeIfAbsent(docId, k -> new ArrayList<>()).add(
                            br.dev.ctrls.itsm.modules.appointment.infrastructure.adapter.output.discord.AppointmentDiscordNotifierService.PatientAttentionItem.builder()
                                    .patientName(pName)
        ...
        try {
            Map<Long, br.dev.ctrls.itsm.modules.appointment.domain.model.DoctorConfiguration> doctorConfigMap = new HashMap<>();
            ...
            for (Map.Entry<Long, Integer> entry : appointmentsCountByDoctor.entrySet()) {
                ...
                br.dev.ctrls.itsm.modules.appointment.domain.model.DoctorConfiguration cfg = doctorConfigMap.get(dId);
                ...
                if (channelId != null && !channelId.isBlank()) {
                    ...
                    List<br.dev.ctrls.itsm.modules.appointment.infrastructure.adapter.output.discord.AppointmentDiscordNotifierService.PatientAttentionItem> attentionItems = attentionByDoctor.get(dId);

                    discordNotifierService.sendDoctorDispatchReport(
                            br.dev.ctrls.itsm.modules.appointment.infrastructure.adapter.output.discord.AppointmentDiscordNotifierService.DoctorDispatchSummary.builder()
                                    .doctorId(dId)
                                    .doctorName(docName)
