...
public interface TicketJpaRepository extends JpaRepository<Ticket, UUID>, JpaSpecificationExecutor<Ticket> {
    ...
    @Query("""
            ...
            """)
    List<br.dev.ctrls.itsm.modules.ticket.domain.model.Ticket> findByRequesterIdOrderByCreatedAtDesc(@Param("requesterId") UUID requesterId);
    ...
    @Query("""
            ...
            JOIN FETCH t.category
            JOIN t.tags tag
            WHERE t.status = br.dev.ctrls.itsm.modules.ticket.domain.model.TicketStatus.RESOLVED
              AND t.id <> :ticketId
              AND tag IN :tags
            """)
