...
import br.dev.ctrls.itsm.modules.asset.domain.model.Asset;
import br.dev.ctrls.itsm.modules.asset.domain.port.output.AssetRepositoryPort;
import br.dev.ctrls.itsm.modules.inventory.domain.model.Item;
import br.dev.ctrls.itsm.modules.ticket.domain.port.output.TicketInventoryGateway;
import br.dev.ctrls.itsm.modules.notification.application.service.CreateNotificationService;
import br.dev.ctrls.itsm.modules.notification.infrastructure.adapter.output.discord.DiscordWebhookService;
import br.dev.ctrls.itsm.modules.notification.infrastructure.adapter.output.discord.bot.DiscordDirectMessageService;
...
import br.dev.ctrls.itsm.modules.ticket.domain.model.TicketTag;
import br.dev.ctrls.itsm.modules.ticket.domain.model.TicketTagExtractor;
...
import br.dev.ctrls.itsm.modules.ticket.domain.port.output.TicketTagRepositoryPort;
import br.dev.ctrls.itsm.modules.ticket.domain.event.TicketCreatedEvent;
...
public class CreateTicketUseCase {
    ...
    public TicketResponseDTO execute(TicketRequestDTO request) {
        ...
        java.util.List<br.dev.ctrls.itsm.modules.ticket.domain.model.TicketItemRequestEntity> reqItems = new java.util.ArrayList<>();
        if (request.requestedItems() != null && !request.requestedItems().isEmpty()) {
            for (var dtoItem : request.requestedItems()) {
                Item item = itemRepository.findById(dtoItem.itemId())
                        .orElseThrow(() -> new br.dev.ctrls.itsm.core.shared.domain.model.exception.NotFoundException("Requested item not found with id: " + dtoItem.itemId()));
                reqItems.add(br.dev.ctrls.itsm.modules.ticket.domain.model.TicketItemRequestEntity.builder()
                        .ticket(ticket)
                        .item(item)
